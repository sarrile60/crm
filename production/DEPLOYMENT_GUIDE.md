...
curl -I https://dialerfix-crm.preview.emergentagent.com/backend/.env
# Should return: 404

curl -I https://dialerfix-crm.preview.emergentagent.com/backend/main.py
# Should return: 404
```
...
```bash
# Health check
curl https://dialerfix-crm.preview.emergentagent.com/health
# Expected: {"status":"healthy"}

# Try to access blocked files (should all be 404)
curl -I https://dialerfix-crm.preview.emergentagent.com/backend/.env
curl -I https://dialerfix-crm.preview.emergentagent.com/backend/main.py
curl -I https://dialerfix-crm.preview.emergentagent.com/.git/config

# Test public lead submission (should work)
curl -X POST https://dialerfix-crm.preview.emergentagent.com/api/leads/submit \
...
# Check admin panel
curl -I https://dialerfix-crm.preview.emergentagent.com/admin
# Expected: 200 OK

# Check static assets are cached
curl -I https://dialerfix-crm.preview.emergentagent.com/static/js/main.*.js
# Expected: Cache-Control: public, immutable
```

### 3. Security Headers Verification
```bash
curl -I https://dialerfix-crm.preview.emergentagent.com/ | grep -E "X-Frame-Options|X-Content-Type|Strict-Transport"
...
for i in {1..10}; do
  curl -X POST https://dialerfix-crm.preview.emergentagent.com/api/crm/auth/login \
    -H "Content-Type: application/json" \
