...
curl -I https://finance-suite-9.preview.emergentagent.com/backend/.env
# Should return: 404

curl -I https://finance-suite-9.preview.emergentagent.com/backend/main.py
# Should return: 404
```
...
```bash
# Health check
curl https://finance-suite-9.preview.emergentagent.com/health
# Expected: {"status":"healthy"}

# Try to access blocked files (should all be 404)
curl -I https://finance-suite-9.preview.emergentagent.com/backend/.env
curl -I https://finance-suite-9.preview.emergentagent.com/backend/main.py
curl -I https://finance-suite-9.preview.emergentagent.com/.git/config

# Test public lead submission (should work)
curl -X POST https://finance-suite-9.preview.emergentagent.com/api/leads/submit \
...
curl -X POST https://finance-suite-9.preview.emergentagent.com/api/admin/login \
  -H "Content-Type: application/json" \
  -d '{
...
curl -I https://finance-suite-9.preview.emergentagent.com/
# Expected: 200 OK

# Check admin panel
curl -I https://finance-suite-9.preview.emergentagent.com/admin
# Expected: 200 OK

# Check static assets are cached
curl -I https://finance-suite-9.preview.emergentagent.com/static/js/main.*.js
# Expected: Cache-Control: public, immutable
```

### 3. Security Headers Verification
```bash
curl -I https://finance-suite-9.preview.emergentagent.com/ | grep -E "X-Frame-Options|X-Content-Type|Strict-Transport"
...
for i in {1..10}; do
  curl -X POST https://finance-suite-9.preview.emergentagent.com/api/crm/auth/login \
    -H "Content-Type: application/json" \
